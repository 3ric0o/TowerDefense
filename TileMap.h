...
struct TileMap
{
  TileMap(const char* mapPath, const char* tilesetPath, int width, int height, int tileSize) :
    width(width), height(height), tileSize(tileSize), scale(2.0f) // Added scale with default 2.0
  {
  ...
  void Draw(int x_offset, int y_offset)
  {
    for (int y = 0; y < height; y++)
    {
      for (int x = 0; x < width; x++)
      {
        ...
        const Rectangle dest = {static_cast<float>(screenX), static_cast<float>(screenY), (tileSize * scale), (tileSize * scale)};
  ...
  // Getter for tile info (useful for interaction)
...
private:
  ...
  int height;
  int tileSize;
  float scale;  // Added scale member variable
};